...
def update_order_status(confirmation_code: str) -> int:
    ...
    logger.info("Buscando pedido pendiente con código: '%s'", code)
    conn = connect_db()
    try:
        with conn.cursor() as cur:
            # Se compara exactamente el código sin espacios
            cur.execute(
            ...
            conn.commit()
            logger.info("Pedido %s actualizado a 'entregado' para el usuario %s", order_id, telegram_id)
            return telegram_id
    except Exception as e:
        logger.error("Error al actualizar el pedido: %s", e)
        conn.rollback()
        return None
    finally:
        release_db(conn)
...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    if not user:
        ...
        await update.message.reply_text("Bienvenido. Solo tiene que registrarse con nombre y dirección para empezar a hacer pedidos a domicilio. Para comenzar, ingrese su nombre:")
        return NAME
...
async def ver_pedido_handler(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    logger.info("Comando /verpedido recibido para el código: '%s'", code)
    ...
    try:
        ...
        logger.error("Error en ver pedido: %s", e)
        await update.message.reply_text("Error al obtener el pedido.")
        return ConversationHandler.END
